...
- [Правила проєкту](#project-rules)
    - [Запис правил](#recording-rules)
    - [Виведення конвенцій вашого застосунку](#inferring-your-applications-conventions)
    - [Вимкнення правил проєкту](#disabling-project-rules)
...
| Record Rule          | Записує довготривале [правило проєкту](#project-rules) у `.ai/rules`, щоб майбутні агенти його успадкували |
...
Коли ви виконуєте `boost:install` і обираєте навички як можливість, вони встановлюються автоматично - за пакетами, виявленими у вашому `composer.json`. Наприклад, якщо ваш проєкт містить `livewire/livewire`, навичку `livewire-development` буде встановлено автоматично. Навички, що йдуть у комплекті з Boost, як-от `infer-conventions`, встановлюються незалежно від того, які пакети у вас є.
...
І настанови, і навички описують екосистему Laravel. Щоб зафіксувати угоди вашого власного застосунку, варто використовувати [правила проєкту](#project-rules).

<a name="project-rules"></a>
## Правила проєкту

Хоча рекомендації та навички навчають агентів писати Laravel, правила проєкту навчають їх писати ваш застосунок. Правило - це все, що вам інакше довелося б пояснювати знову в кожній новій сесії:

<div class="content-list" markdown="1">

- Рішення, прийняті по ходу вами, вашими агентами або вашими колегами.
- Стильові рекомендації та вподобання, яким складно навчити агента слідувати.
- Пастки та обмеження, які не можна вивести з навколишнього коду.

</div>

Правила зберігаються як Markdown-файли в каталозі `.ai/rules` вашого застосунку і мають комітитися до системи контролю версій. На відміну від власної пам'яті агента, яка є персональною та обмеженою сесією, ваші правила спільні з вашою командою і з кожним агентом, що працює над вашим застосунком.

Кожен файл правил оголошує маски файлів, до яких він застосовується, у своєму frontmatter:
...
Крім того, Boost підтримує файл `.ai/rules/index.md`, який зіставляє маски з їхніми файлами правил. Агентам наказано консультуватися з цим індексом перед плануванням або редагуванням будь-якого файлу, тому правило завантажується лише тоді, коли воно є релевантним:
...
> [!NOTE]
> На відміну від `.mcp.json` та згенерованих файлів рекомендацій, каталог `.ai/rules` має комітитися до системи контролю версій, щоб ваші правила були спільними з вашою командою.

<a name="recording-rules"></a>
### Запис правил

Щоб записати правило, ви можете просто попросити свого агента запам'ятати його:

```text
Remember that all money values are stored as integer cents, never as floats.
```

Агент викличе MCP-інструмент `record-rule` Boost з параметрами `glob`, коротким `title` і `note`. Boost потім збереже правило під відповідною областю, створивши файл правила за потреби, і оновить індекс.

Ви завжди повинні записувати правила за допомогою інструменту `record-rule`, а не створювати файли правил вручну. Boost регенерує `.ai/rules/index.md` як частину процесу запису правила, і агенти покладаються на цей індекс для виявлення, які правила застосовуються до файлу, над яким вони працюють. Файл правила, доданий вручну, не буде виявлено, доки індекс не буде регенеровано наступного разу.

<a name="inferring-your-applications-conventions"></a>
### Виведення конвенцій вашого застосунку

Запис правил по одному добре працює в майбутньому; проте, існуючий застосунок уже містить роки конвенцій. Навичка `infer-conventions` підготує ваші правила з коду, який ви вже написали. Для початку попросіть свого агента використати навичку:

```text
Use the infer-conventions skill
```

Навичка проаналізує ваш застосунок за контрольним списком аспектів конвенцій Laravel, включаючи валідацію, контролери, авторизацію, моделі, архітектуру, тестування, фронтенд, базу даних та консоль, а потім виконає відкритий прохід для шаблонів на кшталт базових класів, спільних трейтів та структури модулів.

Навичка документує те, що ваш код насправді робить, а не те, що він повинен робити. Вона записує лише добре підтримані, не-дефолтні конвенції, пропускає налаштування фреймворку за замовчуванням та все, що вже забезпечують Pint або Rector, і повідомляє про справді змішані шаблони замість їх запису. Перед записом будь-яких правил навичка представить кожну виявлену конвенцію разом із підтверджуючими доказами для вашого схвалення. Якщо ви хочете, щоб навичка записала всі виявлені конвенції без підтвердження, ви можете сказати їй "yolo".

<a name="disabling-project-rules"></a>
### Вимкнення правил проєкту

Правила проєкту увімкнено за замовчуванням. Щоб вимкнути їх повністю, визначте наступну змінну оточення. Це видалить інструмент MCP `record-rule` і зупинить керування Boost каталогом `.ai/rules`:
