...
#### Налаштування каналів Single, Daily та Monthly

Канали `single`, `daily` та `monthly` мають три необов'язкові опції конфігурації: `bubble`, `permission` та `locking`.

<div class="overflow-auto">

| Назва        | Опис                                                                          | За замовчуванням |
| ------------ | ----------------------------------------------------------------------------- | ---------------- |
| `bubble`     | Визначає, чи повідомлення мають спливати до інших каналів після обробки.     | `true`           |
| `locking`    | Спроба заблокувати файл журналу перед записом до нього.                      | `false`          |
| `permission` | Права доступу до файлу журналу.                                               | `0644`           |

</div>

Крім того, політику зберігання для каналів `daily` та `monthly` можна налаштувати через опцію конфігурації `max_files`. Змінна оточення `LOG_DAILY_DAYS` також може використовуватися для налаштування зберігання для каналу `daily`.
