...
Канали `single`, `daily` і `monthly` мають три опціональні параметри конфігурації: `bubble`, `permission` і `locking`.

<div class="overflow-auto">

| Назва        | Опис                                                                          | За замовчуванням |
| ------------ | ----------------------------------------------------------------------------- | ------- |
| `bubble`     | Визначає, чи повинні повідомлення передаватися до інших каналів після обробки. | `true`  |
| `locking`    | Намагатися заблокувати файл логу перед записом до нього.                      | `false` |
| `permission` | Права доступу до файлу логу.                                                  | `0644`  |

</div>

Крім того, політику зберігання для каналів `daily` і `monthly` можна налаштувати через параметр конфігурації `max_files`. Змінна оточення `LOG_DAILY_DAYS` також може використовуватися для налаштування зберігання каналу `daily`.
