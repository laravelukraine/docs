...
При оновленні читабельного зашифрованого файлу середовища Laravel зберігає незмінені значення та видаляє змінні, які більше не існують у вихідному файлі середовища:

```shell
php artisan env:encrypt --readable --key=3UVsEgGVK36XN82KKeyLFMhvosbZN1aF
```

Зашифрований файл буде створено, якщо він не існує. При оновленні наявного файлу надайте той самий ключ шифрування та шифр, які використовувалися для його створення. Якщо файл не вдається розшифрувати, команда завершиться з помилкою без його перезапису.

Щоб зашифрувати всі значення знову, використовуйте параметр `--force`. Це дозволяє змінити ключ шифрування або замінити невалідний зашифрований файл:
