...
Альтернативно ви можете описати опцію `report` у масиві конфігурації вашого диска. Коли ця опція має значення `true`, Laravel залогує базовий виняток через обробник винятків вашого застосунку, якщо операція запису провалиться, не викидаючи виняток і не змінюючи значення, яке повертає операція запису:
...
Якщо жодна з опцій `throw` чи `report` не описана, диск мовчки поверне `false` у разі невдачі, і базовий виняток не буде викинуто чи залоговано.
