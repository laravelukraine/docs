...
Альтернативно ви можете описати опцію `report` у масиві конфігурації вашого диска. Коли ця опція має значення `true`, Laravel логуватиме базовий виняток через обробник винятків вашого застосунку, коли операція запису провалюється, не викидаючи виняток і не перериваючи повернення значення операції запису:
...
Якщо жодна з опцій `throw` чи `report` не визначена, диск мовчки поверне `false` при невдачі, і базовий виняток не буде викинуто чи залоговано.
