...
- [Конфігурація](#configuration)
    ...
    - [Скоповані файлові системи, системи лише для читання та системи з наскрізним читанням](#scoped-and-read-only-filesystems)
...
### Скоповані файлові системи, системи лише для читання та системи з наскрізним читанням
...
Диски з наскрізним читанням дозволяють мігрувати файли між дисками без простою. Читаючи файл, Laravel спершу шукає його на основному диску. Якщо файл є лише на резервному, Laravel віддає його звідти й одразу копіює на основний - для наступних запитів:
...
Запис і перегляд каталогів завжди йдуть на основний диск. Перевірка наявності файлу та його метаданих працює з обома дисками й нічого не копіює. Якщо ж копіювання не вдалося, читання за замовчуванням усе одно вважається успішним. Щоб натомість отримати виняток, встановіть опцію конфігурації `throw_on_promotion_failure` у `true`.
...
Альтернативно ви можете описати опцію `report` у масиві конфігурації вашого диска. Коли ця опція має значення `true`, Laravel залогує базовий виняток через обробник винятків вашого застосунку, якщо операція запису провалиться, не викидаючи виняток і не перериваючи значення, що повертається операцією запису:
...
Якщо жодна з опцій `throw` чи `report` не описана, диск тихо поверне `false` у разі невдачі, а базовий виняток не буде викинуто чи залоговано.
...
Ви можете використовувати методи `copyToDisk` і `moveToDisk`, щоб скопіювати чи перемістити файл на інший диск. Шлях вихідного файлу використовується на цільовому диску, якщо ви не передасте третій аргумент:
