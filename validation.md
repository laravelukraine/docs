...
Під час тестування правил валідації, які виконують DNS-запити, як-от `active_url` та `email:dns`, ви можете скористатися методом `Validator::fakeDnsLookups`. Він імітує DNS-запити, зберігаючи решту валідаційної поведінки правил:
...
Поле, що перевіряється, має бути PHP-масивом (`array`), усі ключі якого входять до наведеного списку. Потрібно вказати принаймні один ключ:
...
Валідатор `dns` виконує справжній DNS-запит, щоб підтвердити наявність дійсного MX-запису для домену адреси. Він не визначає, чи існує конкретна поштова скринька.

Оскільки ваші тести не повинні залежати від реальних DNS-запитів, ви можете скористатися методом `Validator::fakeDnsLookups`, щоб [імітувати DNS-запити](#rule-active-url), поки інші необхідні валідації, як-от `rfc`, продовжують виконуватися:
...
Це дозволяє вашому застосунку продовжувати використовувати наявні правила валідації під час тестування:
