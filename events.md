...
- [Слухачі подій у черзі](#queued-event-listeners)
    ...
    - [Слухачі подій з відмовостійкістю](#debounced-event-listeners)
...
У наведеному прикладі повторне надсилання подій `ProductUpdated` для того самого продукту протягом `30` секунд призведе до затримки слухача, щоб обробити лише останню подію. Різні ідентифікатори затримки обробляються незалежно.

Якщо ви хочете обмежити, як довго часто надсилана подія може відкладати слухача, ви можете передати аргумент `maxWait` атрибуту `DebounceFor`:
...
Ви можете налаштувати сховище кешу, яке використовується для відстеження затримок, визначивши метод `debounceVia` у вашому слухачі. Метод отримує екземпляр події і має повернути репозиторій кешу:
...
Слухачі із затримкою та унікальні слухачі є взаємовиключними. Слухач, який використовує атрибут `DebounceFor`, не повинен реалізовувати `ShouldBeUnique`.
