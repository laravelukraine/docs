...
- [Конфігурація `serialization` сесії](#session-serialization-configuration)
...
Контракт `Illuminate\Contracts\Queue\Queue` тепер містить методи перевірки розміру черги, які раніше були оголошені лише в блоках docblock.
...
#### Конфігурація `serialization` сесії

**Ймовірність впливу: низька**

Щоб допомогти запобігти атакам через ланцюги десеріалізації PHP, стандартний каркас застосунку тепер встановлює параметр `serialization` сесії в значення `json` у файлі `config/session.php`.

Якщо ви оновлюєте існуючий застосунок і синхронізуєте файли конфігурації з каркасом Laravel 13, зміна цього значення з `php` на `json` анулює всі активні сесії користувачів.

Якщо ви бажаєте безперебійно зберегти активні сесії під час оновлення, ви повинні залишити це значення встановленим на `php`. Однак, якщо ваш застосунок не зберігає PHP-об'єкти в сесії і ви готові вимагати від користувачів повторної автентифікації, ми рекомендуємо оновити це значення до `json` для покращеної безпеки.
