---
git: 501e8cf7f5e69e26b3c64d1a0656cacef2b4d413
---
# Laravel AI SDK
...
Провайдери, сумісні з OpenAI, підтримують генерацію тексту, стримінг, інструменти, структурований вивід, вкладення зображень, ембединги й транскрипцію. Якщо ваш ендпоїнт потребує додаткових полів у тілі запиту, передайте їх через [опції провайдера](#provider-options).
...
Аналогічно, ви маєте налаштувати модель транскрипції за замовчуванням, щоб використовувати `Transcription` з провайдером, сумісним з OpenAI. Аудіо буде завантажено до маршруту `/audio/transcriptions` кінцевої точки як стандартний multipart-запит:
...
> [!NOTE]
> Провайдери, сумісні з OpenAI, та Groq не підтримують діаризацію. Виклик методу `diarize` при використанні цих провайдерів призведе до винятку.
...
За замовчуванням кожен інструмент, який надає агент, передається провайдеру з кожним запитом. Коли агент надає велику кількість інструментів, це споживає токени і може знизити точність вибору інструментів моделлю. Використовуючи інструмент провайдера `ToolSearch` з OpenAI або Anthropic, ви можете відкласти визначення інструментів так, щоб провайдер завантажував їх лише тоді, коли вони потрібні:
...
Обгорнуті інструменти не потребують жодних змін. Провайдер шукатиме та завантажуватиме їх, коли вони стануть релевантними до промпту, після чого агент зможе викликати їх як будь-який інший інструмент.

При використанні Anthropic аргумент `strategy` може використовуватися для визначення того, як провайдер має шукати відкладені інструменти. Підтримувані стратегії - `regex` (за замовчуванням) та `bm25`:

```php
new ToolSearch(tools: [new SearchInvoices], strategy: 'bm25'),
```

При використанні Anthropic додаткові опції, специфічні для провайдера, можна передати інструменту пошуку через метод `withProviderOptions`:
...
> [!WARNING]
> Провайдери, які не підтримують пошук інструментів, викинуть виключення замість того, щоб мовчки відкинути відкладені інструменти. Крім того, Anthropic вимагає, щоб принаймні один інструмент був наданий поза обгорткою `ToolSearch`.
...
Коли ви підміняєте чергові зображення, аудіо, транскрипцію або генерацію вбудовувань, будь-який колбек `then`, зареєстрований на черговій генерації, буде викликаний із підробленою відповіддю, що дозволяє перевірити логіку всередині колбека. Якщо ви воліте, щоб ці колбеки не викликалися, можете додатково підмінити чергу за допомогою `Queue::fake()`.
