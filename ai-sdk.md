...
Хоча схема вашого інструмента обмежує аргументи, які модель може надати, ви можете валідувати вхідні аргументи за допомогою методу `validate` запиту:
...
Коли валідація не проходить, повідомлення про валідацію повертаються моделі як результат інструмента, дозволяючи їй виправити аргументи та викликати інструмент знову.
...
Більшість провайдерів автоматично кешують повторювані префікси промптів і обліковують кешовану частину зі знижкою. OpenAI, Gemini, Groq, DeepSeek та xAI не потребують налаштування, і ви можете перевірити заощадження через використання відповіді:
...
Провайдери `anthropic` і `bedrock` кешують лише за запитом. Атрибути `CacheInstructions` і `CacheToolDefinitions` розміщують точку зупинки кешу в кінці інструкцій вашого агента і визначень інструментів, тож кожна розмова зчитує цей префікс із кешу замість того, щоб записувати його знову:
...
Якщо ваші інструкції змінюються з кожним запитом, наприклад, коли вони містять поточну дату, використовуйте лише `CacheToolDefinitions`. Кешування префіксу, який змінюється з кожним запитом, створює новий запис кешу щоразу, тож ви платите за його запис у кеш, але ніколи не використовуєте повторно.

Провайдери, які не підтримують ці атрибути, ігнорують їх, тому агент може безпечно їх оголошувати при використанні [резервного варіанта](#failover).
...
Як альтернативу, автоматичне кешування Anthropic можна увімкнути через [опцію провайдера](#provider-options) верхнього рівня `cache_control`. Це розміщує одну точку зупинки після останнього блоку запиту, тож точка зупинки просувається в міру зростання розмови, і кожен хід зчитує попередні ходи з кешу. Обидва механізми можна комбінувати.

> [!WARNING]
> Оскільки провайдери будують промпти в порядку інструментів, інструкцій та повідомлень, кешування інструкцій на годину також вимагає кешування визначень інструментів на годину. Змішування цих двох варіантів викидає `InvalidArgumentException`.
...
За замовчуванням ембединг кожного входу кешується під власним ключем, тому наступний запит може отримати результат з кешу для входів, які він уже бачив раніше, навіть якщо набір входів або їхній порядок змінилися. Щоб натомість кешувати весь набір входів під одним ключем, встановіть параметр конфігурації `ai.caching.embeddings.individually` у `false`.
