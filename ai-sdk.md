...
Коли валідація не проходить, повідомлення валідації повертаються моделі як результат інструменту, дозволяючи їй виправити аргументи та викликати інструмент знову.
...
Більшість провайдерів автоматично кешують повторювані префікси промптів і виставляють рахунок за кешовану частину зі знижкою. OpenAI, Gemini, Groq, DeepSeek та xAI не потребують налаштування, і ви можете перевірити економію через використання відповіді:
...
Провайдери `anthropic` і `bedrock` кешують лише на вимогу. Атрибути `CacheInstructions` і `CacheToolDefinitions` встановлюють точку розриву кешу в кінці інструкцій вашого агента та визначень інструментів, тому кожна розмова зчитує цей префікс із кешу замість того, щоб записувати його знову:
...
Якщо ваші інструкції змінюються з кожним запитом, наприклад, коли вони містять поточну дату, використовуйте лише `CacheToolDefinitions`. Кешування префікса, який змінюється з кожним запитом, створює новий запис кешу щоразу, тому ви платите за запис до кешу, не використовуючи його повторно.
...
Кешовані префікси зберігаються п'ять хвилин за замовчуванням. Anthropic може зберігати їх протягом години, якщо ви передасте TTL до атрибута:
...
Як альтернатива, автоматичне кешування Anthropic можна ввімкнути через верхньорівневу [опцію провайдера](#provider-options) `cache_control`. Це встановлює єдину точку розриву після останнього блоку запиту, тож точка розриву просувається в міру зростання розмови, і кожен хід зчитує попередні ходи з кешу. Обидва механізми можна комбінувати.
...
За замовчуванням ембединг кожного вхідного значення кешується під власним ключем, тож наступний запит може взяти з кешу вхідні дані, які він бачив раніше, навіть якщо набір вхідних даних або їх порядок змінився. Щоб натомість кешувати весь набір вхідних даних під одним ключем, встановіть параметр конфігурації `ai.caching.embeddings.individually` у `false`.
