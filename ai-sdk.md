...
Так само потрібно налаштувати модель транскрипції за замовчуванням, щоб використовувати `Transcription` з провайдером, сумісним з OpenAI. Аудіо буде завантажено на маршрут `/audio/transcriptions` кінцевої точки як стандартний multipart-запит:
...
> [!NOTE]
> Провайдери, сумісні з OpenAI, та Groq не підтримують діаризацію. Виклик методу `diarize` під час використання цих провайдерів призведе до винятку.
...
За замовчуванням кожен інструмент, який надає агент, відправляється провайдеру з кожним запитом. Коли агент надає велику кількість інструментів, це споживає токени і може знизити точність вибору інструментів моделлю. Використовуючи інструмент провайдера `ToolSearch` з OpenAI або Anthropic, ви можете відкласти визначення інструментів так, щоб провайдер завантажував їх тільки тоді, коли вони потрібні:
...
Обгорнуті інструменти не потребують жодних модифікацій. Провайдер шукатиме і завантажуватиме їх, коли вони релевантні до промпту, після чого агент може викликати їх як будь-який інший інструмент.

При використанні Anthropic аргумент `strategy` може використовуватися для визначення того, як провайдер має шукати відкладені інструменти. Підтримувані стратегії - це `regex` (за замовчуванням) і `bm25`:

```php
new ToolSearch(tools: [new SearchInvoices], strategy: 'bm25'),
```

При використанні Anthropic додаткові опції, специфічні для провайдера, можуть бути передані до інструмента пошуку за допомогою методу `withProviderOptions`:
...
> [!WARNING]
> Провайдери, які не підтримують пошук інструментів, викинуть виняток замість того, щоб мовчки відкидати відкладені інструменти. Крім того, Anthropic вимагає, щоб принаймні один інструмент був наданий поза обгорткою `ToolSearch`.
...
Коли ви фейкаєте генерацію зображень, аудіо, транскрипцію або ембедингів у черзі, будь-який колбек `then`, зареєстрований на генерації в черзі, буде викликаний з фейковою відповіддю, що дозволяє вам тестувати логіку всередині колбека. Якщо ви бажаєте, щоб ці колбеки не викликалися, ви можете також зафейкати чергу за допомогою `Queue::fake()`.
