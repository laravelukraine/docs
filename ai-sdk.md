...
#### Транскрипція з OpenAI-сумісними провайдерами

Аналогічно, ви маєте налаштувати модель транскрипції за замовчуванням, щоб використовувати `Transcription` з OpenAI-сумісним провайдером. Аудіо буде завантажено на маршрут `/audio/transcriptions` кінцевої точки як стандартний multipart-запит:
...
> [!NOTE]
> OpenAI-сумісні провайдери та Groq не підтримують діаризацію. Виклик методу `diarize` при використанні цих провайдерів викине виняток.
...
Обгорнуті інструменти не потребують жодних змін. Провайдер шукатиме й завантажуватиме їх, коли вони будуть релевантні до промпту, після чого агент зможе викликати їх як будь-який інший інструмент.

При використанні Anthropic аргумент `strategy` можна використовувати для визначення того, як провайдер має шукати відкладені інструменти. Підтримувані стратегії - `regex` (за замовчуванням) і `bm25`:

```php
new ToolSearch(tools: [new SearchInvoices], strategy: 'bm25'),
```

При використанні Anthropic додаткові опції, специфічні для провайдера, можна передати до інструменту пошуку за допомогою методу `withProviderOptions`:
...
Коли ви фейкаєте поставлене в чергу генерування зображень, аудіо, транскрипцій або ембедингів, будь-який колбек `then`, зареєстрований на поставленому в чергу генеруванні, буде викликаний з фейковою відповіддю, дозволяючи вам тестувати логіку всередині колбека. Якщо ви віддаєте перевагу тому, щоб ці колбеки не викликалися, ви можете також зафейкати чергу за допомогою `Queue::fake()`.
