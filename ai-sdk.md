...
#### Транскрипція, сумісна з OpenAI

Аналогічно, ви маєте налаштувати модель транскрипції за замовчуванням, щоб використовувати `Transcription` з провайдером, сумісним з OpenAI. Аудіо буде завантажено на маршрут `/audio/transcriptions` точки доступу як стандартний multipart-запит:
...
Обгорнуті інструменти не потребують жодних змін. Провайдер шукатиме та завантажуватиме їх, коли вони відповідатимуть запиту, після чого агент зможе викликати їх, як будь-який інший інструмент.

При використанні Anthropic аргумент `strategy` можна використовувати для визначення того, як провайдер має шукати відкладені інструменти. Підтримуються стратегії `regex` (за замовчуванням) і `bm25`:

```php
new ToolSearch(tools: [new SearchInvoices], strategy: 'bm25'),
```

При використанні Anthropic додаткові опції, специфічні для провайдера, можна передати до інструмента пошуку за допомогою методу `withProviderOptions`:
...
Під час фейкування черги для генерації зображень, аудіо, транскрипцій або embeddings будь-який колбек `then`, зареєстрований на генерації в черзі, буде викликано з підробленою відповіддю, що дозволяє тестувати логіку всередині колбека. Якщо ви не хочете, щоб ці колбеки викликалися, можете також підробити чергу за допомогою `Queue::fake()`.
