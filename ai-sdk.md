...
Провайдери, які не підтримують ці атрибути, ігнорують їх, тож агент може безпечно оголошувати їх під час використання [failover](#failover).
...
Альтернативно, автоматичне кешування Anthropic може бути ввімкнено через опцію `cache_control` верхнього рівня [провайдера](#provider-options). Це розміщує одну точку розриву після останнього блоку запиту, тож точка розриву просувається в міру зростання розмови, і кожен хід зчитує попередні ходи з кешу. Обидва механізми можуть поєднуватися.
