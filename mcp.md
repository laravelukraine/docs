...
- [Створення серверів](#creating-servers)
    ...
    - [Підказки кешування](#cache-hints)
- [Інструменти](#tools)
    - [Створення інструментів](#creating-tools)
    - [Каталоги інструментів із пошуком](#searchable-tool-catalogs)
...
Сервери з великою кількістю інструментів можуть розмістити частину з них у каталозі з пошуком, замість оголошувати кожен AI-клієнту. Каталог із пошуком надає два інструменти: `search_tools`, який шукає в каталозі за назвою, описом і схемою вхідних даних; і `execute_tools`, який викликає один або кілька знайдених.
...
У цьому прикладі `CurrentWeatherTool` оголошується безпосередньо, тоді як інструменти історичної погоди та погодних сповіщень доступні через каталог із пошуком. Умовна реєстрація інструментів все ще враховується, коли інструменти з каталогу шукаються або виконуються.
...
> [!NOTE]
> Аргументи `clientId` і `clientSecret` можна опустити. Laravel використає [Client ID Metadata Document](https://modelcontextprotocol.io/specification/2026-07-28/basic/authorization/client-registration#client-id-metadata-documents), коли сервер авторизації їх підтримує, і повертається до [динамічної реєстрації клієнтів](https://datatracker.ietf.org/doc/html/rfc7591) для застарілих серверів.
