...
Artisan - це інтерфейс командного рядка, що входить до складу Laravel. Artisan лежить у корені вашого застосунку як скрипт `artisan` і надає чимало корисних команд, які можна використовувати під час розробки. Щоб побачити список усіх доступних команд Artisan, скористайтеся командою `list`:
...
Під капотом команда `dev` керує процесами через npm-пакет `@laravel/multiplex`, відкриваючи для кожного процесу власну вкладку з виводом, який можна прокручувати і в якому можна шукати. Кожен процес має свою мітку й колір, тож їх легко розрізнити. Якщо якийсь процес зазнає краху, його буде автоматично перезапущено, а коли ви завершите роботу, весь вивід буде записано назад у термінал, тож нічого не втрачається.

> [!NOTE]
> Команда `dev` вимагає Node 22.13 або новішої версії. На Windows вона повертається до npm-пакета `concurrently`, і інтерфейс із вкладками недоступний.
...
#### Перезапуск процесів, що завершилися з помилкою

Якщо процес аварійно завершується, Laravel перезапустить його після короткої затримки, до п'яти разів, перш ніж позначити його як невдалий. Процес, який припиняється протягом секунди після запуску, не перезапускається, оскільки ймовірно він ніколи не запускався успішно з самого початку. Ручний перезапуск процесу через `r` скидає лічильник.

Ви можете вимкнути цю поведінку для одного запуску, використовуючи опцію `--no-restart`:

```shell
php artisan dev --no-restart
```

Або ви можете вимкнути її для всього застосунку, використовуючи метод `disableAutoRestart`:
