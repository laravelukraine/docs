...
Опція конфігурації `autoScalingStrategy` визначає, як Horizon призначатиме чергам більше процесів-воркерів. Ви можете обрати одну з трьох стратегій:
...
- Стратегія `log` призначатиме воркерів за логарифмом кількості завдань у черзі. Це запобігає тому, щоб істотно більша черга отримувала непропорційно велику частку воркерів.
